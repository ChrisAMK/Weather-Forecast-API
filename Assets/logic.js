$( document ).ready(function() {
    ...
    function init() {
        ...
        var allStoredHistory = JSON.parse(localStorage.getItem("allHistory"));
        if (allStoredHistory !== null) {
            allPastSearches = allStoredHistory
        }
    ...
    function storeAllPastSearches() {
        localStorage.setItem("allHistory", JSON.stringify(allPastSearches));
    }
    ...
    searchBtn.on("click", function(event) {
        ...
        searchCity()
        searchCityForecast()
        // declaring what the user types as a variable
        var pastSearch = userInput.val().trim()
        // This is a array that tracks all searches even if its a repeat, for page freshing purposes
        allPastSearches.push(pastSearch)
        storeAllPastSearches()
        
        // if the user submits something that already exist, it wont be re-added to the History Bar.
    ...
    function showLastCityForecast() {
        // This grabs the Users input for which city they want to search.
        var searchTerm = allPastSearches.slice(-1)
        var key = "bda2f2ea374379c994c54cc335a5e52b";
        var queryURL = "https://api.openweathermap.org/data/2.5/forecast?q=" + searchTerm + "&units=metric&APPID=" + key;
        // Ajax Call grabs the information from the API and presents the data in an object
        $.ajax({
